...
std::string Token::getKindAsString() {
  ...
  switch (kind) {
    case Token::ID:         kindString = "ID";         break;
    case Token::LABEL:      kindString = "LABEL";      break;
    case Token::WORD:       kindString = "WORD";       break;
    case Token::COMMA:      kindString = "COMMA";      break;
    case Token::LPAREN:     kindString = "LPAREN";     break;
    case Token::RPAREN:     kindString = "RPAREN";     break;
    case Token::INT:        kindString = "INT";        break;
    case Token::HEXINT:     kindString = "HEXINT";     break;
    case Token::REG:        kindString = "REG";        break;
    case Token::WHITESPACE: kindString = "WHITESPACE"; break;
    case Token::COMMENT:    kindString = "COMMENT";    break;
  }
  return kindString;
}
...
std::pair<std::vector<Token>, std::set<std::string>> scan(const std::string &input) {
  ...
  // Stefan modified this
  std::set<std::string> instructionTypes;

  for (auto &token : tokens) {
    if (token.getKind() == Token::WORD) {
      if (token.getLexeme() == ".word") {
        newTokens.push_back(token);
        instructionTypes.emplace("word");
      } else {
      ...
      if (token.getKind() == Token::LABEL) {
        instructionTypes.emplace("label");
      }
      else if (token.getKind() == Token::ID) {
        if (token.getLexeme() == "add") {
          instructionTypes.emplace("add");
        }
        else if (token.getLexeme() == "sub") {
          instructionTypes.emplace("sub");
        }
        else if (token.getLexeme() == "slt") {
          instructionTypes.emplace("slt");
        }
        else if (token.getLexeme() == "sltu") {
          instructionTypes.emplace("sltu");
        }
        else if (token.getLexeme() == "mult") {
          instructionTypes.emplace("mult");
        }
        else if (token.getLexeme() == "multu") {
          instructionTypes.emplace("multu");
        }
        else if (token.getLexeme() == "div") {
          instructionTypes.emplace("div");
        }
        else if (token.getLexeme() == "divu") {
          instructionTypes.emplace("divu");
        }
        else if (token.getLexeme() == "mfhi") {
          instructionTypes.emplace("mfhi");
        }
        else if (token.getLexeme() == "mflo") {
          instructionTypes.emplace("mflo");
        }
        else if (token.getLexeme() == "lis") {
          instructionTypes.emplace("lis");
        }
        else if (token.getLexeme() == "lw") {
          instructionTypes.emplace("lw");
        }
        else if (token.getLexeme() == "sw") {
          instructionTypes.emplace("sw");
        }
        else if (token.getLexeme() == "beq") {
          instructionTypes.emplace("beq");
        }
        else if (token.getLexeme() == "bne") {
          instructionTypes.emplace("bne");
        }
        else if (token.getLexeme() == "jr") {
          instructionTypes.emplace("jr");
        }
        else if (token.getLexeme() == "jalr") {
          instructionTypes.emplace("jalr");
        }
      }
    }
  }

  std::pair<std::vector<Token>, std::set<std::string>> tokensAndInstructionTypes(newTokens, instructionTypes);

  return tokensAndInstructionTypes;
}
